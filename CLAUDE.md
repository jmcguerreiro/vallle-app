...
- **Closing**: Pressing Escape, clicking the backdrop, or the close button return to whatever page was behind the modal. When the modal was opened in-app there's history to go back to (`navigate(-1)`); when it was opened directly there isn't, so `RouteModal` falls back to navigating to the dashboard (`ROUTES.HOME`).
- **Direct access**: If someone lands on a modal URL directly (no background location), `AppRoutes` synthesizes the dashboard (`ROUTES.HOME`) as the background — so the modal still renders on top of the dashboard instead of as a full page. The list of modal paths lives in `MODAL_ROUTE_PATHS` in `App.jsx`; add new modal routes there. Because direct access now flows through the modal `<Routes>` block (not page routes), that block carries its own guards: it's wrapped in `AuthGuard`, and admin modals are wrapped in `AdminRoute`. There are **no** fallback page routes for modal URLs — the modal block is the single source of truth.
