...
| 2026-06-02 | Adopt Prettier defaults (double quotes + semicolons) | The codebase had drifted between two styles because no `.prettierrc` existed and `eslint-config-prettier` only *disables* style rules. Added `.prettierrc.json` + `npm run format` and reformatted `src/`. CLAUDE.md §12 updated to defer formatting to Prettier (dropped the old single-quote / no-semicolon rule) |
