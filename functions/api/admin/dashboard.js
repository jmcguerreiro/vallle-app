...
export async function onRequestGet(context) {
  ...
  try {
    const [storesRow, valllesRow, commissionsRow, monthlyVallles, monthlyCommissions] = await Promise.all([
      env.DB.prepare('SELECT COUNT(*) AS count FROM stores').first(),
      env.DB.prepare('SELECT COALESCE(SUM(amount), 0) AS total FROM vallles').first(),
