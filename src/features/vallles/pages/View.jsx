...
const VallleView = () => {
  ...
  const fetchVallle = useCallback(async () => {
    ...
    try {
      const [vallleRes, redemptionsRes] = await Promise.all([
        get(`/api/vallles/${id}`),
        get(`/api/vallles/${id}/redemptions`),
      ])
      setVallle(vallleRes.data)
  ...
  const fields = [
    ...
    { label: t('features.vallles.view.expiresAt'), value: formatDate(vallle.expires_at) },
    { label: t('features.vallles.view.createdAt'), value: formatDate(vallle.created_at) },
  ]
  ...
  return (
    <div className="p-vallle-view">
      <div className="p-vallle-view__hero">
        ...
        <h2 className={`p-vallle-view__code${statusKey === 'active' ? '' : ' p-vallle-view__code--inactive'}`}>
          {vallle.code}
        </h2>
      ...
      <Accordion className="c-vallle-redemptions" title={t('features.vallles.redemptions.heading')}>
